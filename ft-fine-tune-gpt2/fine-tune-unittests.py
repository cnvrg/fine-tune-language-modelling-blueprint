import pandas as pd
import unittest
from finetune import DataTrainingArguments, get_dataset, ModelArguments
import os
...
class TestFT(unittest.TestCase):
    ...
    def test_outputdir_exist(self):
        ...
        file = pl.Path("./story_generator_checkpoint_" + "gpt2")
    ...
    def test_return_metrics(self):
        ...
        data_args = DataTrainingArguments(
            train_data_file='6_genre_clean_training_data_small.txt',
            eval_data_file='6_genre_eval_data_small.txt',
            line_by_line=True,
            block_size=256,
            overwrite_cache=True,
        )
        ...
        config = AutoConfig.from_pretrained(model_name, cache_dir=model_args.cache_dir)
        tokenizer = AutoTokenizer.from_pretrained(model_name, cache_dir=model_args.cache_dir)
        ...
        self.assertLess(perplexity, 160, message)
